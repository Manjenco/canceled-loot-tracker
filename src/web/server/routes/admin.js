...
router.put('/seasons/:id', requireGlobalOfficer, async (c) => {
  ...
  let body;
  try { body = await c.req.json(); } catch { return c.json({ error: 'Invalid JSON body' }, 400); }
  const { name, startDate, mplusWse, preRelease, zoneIds, tokenSlotWords, veteranBonusId } = body ?? {};
  try {
    await updateSeason(db, id, { name, startDate, mplusWse, preRelease, zoneIds, tokenSlotWords, veteranBonusId });
    return c.json({ ok: true });
  } catch (err) {
